...
async function processQueue(channelId: string) {
  ...
  try {
    ...
    // ── NARRATOR SENTENCE FILTER ───────────────────────────────────────────────────
    // The model keeps producing "reporter-style" observations about the conversation
    // despite system-prompt bans. This strips them on a sentence-by-sentence basis.
    function filterNarratorSentences(text: string): string {
      ...
      const patterns = [
        ...
        /\bno hay una pregunta clara para mí\b/i,
        /\bpuedo ofrecer una respuesta breve\b/i,
        /\bconversaci[oó]n un poco ca[oó]tica\b/i,
      ];
      ...
      const filtered = sentences.filter(s => {
        ...
        if (matches) {
          console.warn(`[NARRATOR STRIP] Stripped sentence: "${trimmed}"`);
        }
        return !matches;
      });
    ...
    let filteredReply = filterNarratorSentences(safeReply);

    // If after filtering the reply is empty:
    // - If it was a lurk response: skip it silently
    // - If directly mentioned: fallback to a generic dry response so we don't send an empty reply or stay silent
    if (!filteredReply) {
      if (!wasDirectlyMentioned) {
        console.warn(`[NARRATOR FILTER] Skipping lurk response — reply was empty after narrator strip.`);
        return;
      } else {
        console.warn(`[NARRATOR FILTER] Direct mention reply was empty after narrator strip — using fallback.`);
        const fallbacks = ["Dramático.", "No te pille.", "Qué.", "Ya veo."];
        filteredReply = fallbacks[Math.floor(Math.random() * fallbacks.length)];
      }
    }
    ...
    if (wasDirectlyMentioned) {
      await lastMessage.reply(filteredReply);
    } else {
      await (lastMessage.channel as any).send(filteredReply);
    }
